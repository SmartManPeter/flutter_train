...
String formatTaiwanDateTime(DateTime time, bool showFuzzy) {
  ...
  if (showFuzzy) {
    ...
    if (difference.inMinutes.abs() < 5) {
      fuzzyStr = diffMinutes < 0 ? "剛剛" : "即將發生";
    } else if (difference.inHours.abs() < 1) {
      fuzzyStr = diffMinutes < 0 ? "${diffMinutes.abs()}分鐘前" : "${diffMinutes.abs()}分鐘後";
    } else if (difference.inDays.abs() < 1) {
      fuzzyStr = diffHours < 0 ? "${diffHours.abs()}小時前" : "${diffHours.abs()}小時後";
    } else if (difference.inDays.abs() < 7) {
      fuzzyStr = diffDays < 0 ? "${diffDays.abs()}天前" : "${diffDays.abs()}天後S";
    } else {
      fuzzyStr = ""; // 超過一週不顯示
    }
